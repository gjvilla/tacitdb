...
| U-31 | Whose signature counts. D-0025 takes git's own verdict on a signature and asks no further: there is no registry of keys the project trusts, no notion of which humans may carry verdicts, and no defence against a signature made on an unlocked machine by something that is not its owner | When a second person edits the corpus, or when an agent is given write access to it | The residue of U-29, and the same question D-0012 assigned to the keeper: "authentication and authorization of *who* counts belong to the keeper layer above". D-0025 is the first thing that needed it and got by without it. Note the honest limit of any answer here: no signing scheme survives a compromised endpoint, so the goal is attribution a reader can weigh, not proof. |
