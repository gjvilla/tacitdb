...
| U-26 | Approximate nearest neighbours: vector search is an exact scan over admitted records | When the corpus makes the scan cost measurable — **fired 2026-08-24** | **Measured (D-0030, refined by D-0031):** a hybrid query costs 1.2ms at a thousand claims, 6.6ms at eight thousand and 29ms at sixty-eight thousand records — 125x the lexical half. The scan itself is only part of it: the plan materialises every candidate to return ten, so fusion and item construction scale with the index rather than with the answer. Capping the fusion depth would cut that and is an approximation; an approximate index returns the top directly and subsumes the cap, which is why the cap has not been done separately. HNSW with predicate-aware traversal is the shape the priors point at. |
