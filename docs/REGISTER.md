...
| U-28 | The verdict grammar has no supersession path for a gap or a hypothesis. A claim that is reworded is superseded and retired in one verdict; a *question* that is reworded has nothing equivalent, so the ledger keeps the wording it has and the sync reports drift | With U-16, or the first register rewording that matters | Found by implementing D-0021 — the sync could express "this decision now reads differently" and could not express "this question now reads differently". The shape of a fix is visible: `Withdraw` gains a reason the way `Retire` has one, so "we stopped asking" and "we asked it better" stop being the same recorded event. That is a grammar change and therefore a decision record, not an implementation detail. |
| U-29 | The upstream document is an unaudited promotion channel. Ingest transcribes `state: promoted` as a human verdict attributed to the record's stated author, and the ingester cannot know who edited the file — so write access to `docs/DECISIONS.md` is promotion authority, with no promote tool anywhere in sight | Before any corpus an agent can write to, and before U-7 clears public release | The MCP surface has no promote tool and an integration test holds that line. This is the side door: an agent that can edit the repository can promote by editing prose. Live from 2026-08-24, because D-0021 made re-ingest routine — before it, the host read the documents exactly once. Not fixable by a stronger fingerprint (whoever can forge a collision can simply rewrite the file); the missing piece is an authenticity signal on the transcription channel — signed commits, or a keeper-side attestation — which is D-0012's "identity and auth live in the keeper", now with a concrete first customer. |
