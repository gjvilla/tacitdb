...
| U-35 | Retrieval admits candidates one at a time, and each admission is a state fold over that record's verdicts. The lexical path pays it only for postings its query terms reach; the vector path pays it for every record in the index | With U-26 | Separated out from U-26 while measuring D-0030, because it is a different cost with a different fix: an approximate index makes the *scan* smaller and leaves the per-candidate fold exactly where it is. A view that materialised its admitted set once would answer it, at the price of the cheap caller-held view D-0016 chose deliberately — so it is a trade, not an oversight. |
...
| U-26 | Approximate nearest neighbours: vector search is an exact scan over admitted records | When the corpus makes the scan cost measurable — **fired 2026-08-24** | **Measured (D-0030):** the vector half costs 6x the lexical one at 1,000 records, 73x at 17,000 and 135x at 68,000 — 39ms per query. And the scan is not the whole of it: every candidate also pays a state fold to be admitted, which the lexical path pays only for the postings its query terms reach. Exact search is still correct and still pre-filters rather than post-filters (R-1). HNSW with predicate-aware traversal is the shape the priors point at, and the admission cost may want its own answer. |
