...
| U-30 | A superseded claim that was never promoted is never closed. `Promote { retiring }` can only retire a claim that is promoted, so a proposed claim replaced by a newer proposed one keeps its `supersedes` link and its place in the pending queue, and only a human `Reject` clears it | When the pending queue is long enough that reviewing it is work | Found while testing D-0023: rewording a `registered` hypothesis leaves two proposed title claims. Arguably correct — nobody has looked at either version, so nothing has been decided about them — but the queue accumulates, and a reviewer seeing two wordings of one record cannot tell which the document currently says. Related in shape to U-18: the cost of never removing anything. |
