...
| U-36 | Neighbourhood buckets are built and kept for a facility that is off by default: eight tables of signatures over every vector, paid at index time and in memory, for a probe nothing calls | When the buckets are either turned on or shown to be unwanted | The honest cost of D-0032. Building them raised the vector index rebuild from about 2.0s to 2.4s at 35,480 vectors and holds eight record-id lists per vector. Cheap enough to keep a measured option available, and not free — the alternatives are building them lazily on first probe, or deciding the approximation is not worth keeping at all. Deferred rather than guessed, because which way it goes depends on whether a real model widens the similarity range enough to make the probe work. |
