...
- **[GOLDEN.md](GOLDEN.md)** — the golden suite, which now audits itself twice
  over (2026-08-24, D-0028 and D-0029): the build fails when a question rests on
  a review trigger that has already fired, and when the corpus quotes a question
  back at it. Four fired triggers and two quotes had accumulated unnoticed, and
  one question was passing on two errors cancelling. `cargo run -p tacit-keeper --example
...
| U-7 | IP clarity: does the employment agreement's invention-assignment clause reach this project, and if not, will that be confirmed in writing? | Before any public release; ideally sooner | **Sharpened 2026-08-24 (D-0035), still open, and only partly mine to close.** Two facts found by the project's own checks and recorded rather than tidied: every commit so far is authored and signed under an employer email address, and nineteen of twenty timestamps fall on a weekday afternoon. The boundary script now reads commit identity as well as file contents — failing on what the next commit would carry, reporting what history carries — and the repository is red until an identity is set for it. History is deliberately not rewritten: restamping the authorship of a project whose ownership is the question would be tidying evidence. [DISCLOSURE.md](DISCLOSURE.md) is the factual page for the conversation. The signing key's UID carries the same address, so every attestation D-0026 records names that identity as the signer — noted, and the keyring left alone. The check is expected to stay red until this resolves: U-7 already blocks publishing, so the gate is now mechanical rather than remembered. What remains is a lawyer's and an employer's, not an engineer's. |
...
| U-27 | Corpus self-reference distorts its own measurement: a record describing a retrieval failure matches the queries that fail | Whenever the golden suite or the register is edited | Found the hard way twice. First when a register row quoted a test question and outranked the record that question was about. Again on 2026-08-24, when two records written in one commit quoted a question and named another's rare words — moving one question's reach from 0.52 to 1.00, so the corpus answered it with the note explaining why it could not. **A third time on 2026-08-24, while writing D-0036** — two ordinary words of prose were rare terms of a golden question, and their arrival lifted it from failing to passing. The first case where contamination *flattered* the result, which is the dangerous direction: a score moving the wrong way gets investigated and one moving the right way does not. Caught by reading the diagnostic instead of banking the number. **Half-mechanised 2026-08-24 (D-0029):** the suite fails the build when a record repeats five or more of a question's words in a row, and the rule is now to name questions by id. The check catches phrases and *cannot* catch vocabulary — writing about a term at all makes it present — so the structural answer is still a corpus the record does not describe, which is U-9. |
