...
| U-32 | Trust is a history, not a current view. An attestation is evaluated once and written into an immutable verdict, so a key revoked tomorrow does not demote what it signed today, and `unattested_promotions` reads the stored account rather than re-asking git | When a key that signed part of this corpus is revoked, or when an audit must answer "what would we believe today?" | Correct as a record of what was known at the time, and incomplete as an answer to what is believed now. Both readings are wanted and only one exists. A re-check is buildable — the commit is named in every attestation — but it makes an audit depend on the repository still being present and the commit still reachable, which a rebase or a garbage collection can end. Found by writing D-0026, not by using it. |
