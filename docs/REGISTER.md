...
| U-27 | Corpus self-reference distorts its own measurement: a record describing a retrieval failure matches the queries that fail | Whenever the golden suite or the register is edited | Found the hard way twice. First when a register row quoted a test question and outranked the record that question was about. Again on 2026-08-24, when two records written in one commit quoted a question and named another's rare words — moving one question's reach from 0.52 to 1.00, so the corpus answered it with the note explaining why it could not. **Half-mechanised 2026-08-24 (D-0029):** the suite fails the build when a record repeats five or more of a question's words in a row, and the rule is now to name questions by id. The check catches phrases and *cannot* catch vocabulary — writing about a term at all makes it present — so the structural answer is still a corpus the record does not describe, which is U-9. |
